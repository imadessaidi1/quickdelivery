...
export const config = {
  ...
  authBaseUrl: (__ENV.AUTH_BASE_URL || 'https://auth.quickdelivery.fr/auth').replace(/\/$/, ''),
  oauthClientId: __ENV.OAUTH_CLIENT_ID || 'quickdelivery-postman',
  oauthClientSecret: __ENV.OAUTH_CLIENT_SECRET || '',
  oauthRealm: __ENV.OAUTH_REALM || 'quickdelivery',
  ...
  adminUsername: __ENV.ADMIN_USERNAME || '',
  adminPassword: __ENV.ADMIN_PASSWORD || '',
  adminBearerToken: __ENV.ADMIN_BEARER_TOKEN || '',
  courierUsername: __ENV.COURIER_USERNAME || '',
  courierPassword: __ENV.COURIER_PASSWORD || '',
