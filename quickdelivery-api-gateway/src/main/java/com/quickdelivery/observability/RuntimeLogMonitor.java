...
public class RuntimeLogMonitor extends AppenderBase<ILoggingEvent> {
    ...
    public void initialize() {
        ...
        setName("quickdelivery-gateway-runtime-log-monitor");
        start();
    ...
    public GatewayServiceLogInsightsResponse snapshot(String serviceName) {
        long now = System.currentTimeMillis();
        long lastHourCutoff = now - (60L * 60L * 1000L);
        synchronized (lock) {
            prune(now);
            List<LoggedEvent> currentEvents = new ArrayList<>(events);
            GatewayServiceLogInsightsResponse insights = new GatewayServiceLogInsightsResponse();
            insights.setServiceName(serviceName);
            insights.setWarnCountLastHour(currentEvents.stream().filter(event -> event.timestamp >= lastHourCutoff && "WARN".equals(event.level)).count());
            insights.setErrorCountLastHour(currentEvents.stream().filter(event -> event.timestamp >= lastHourCutoff && "ERROR".equals(event.level)).count());
            insights.setCategoryCounts(currentEvents.stream().collect(Collectors.groupingBy(event -> event.category, LinkedHashMap::new, Collectors.counting())));
            insights.setHourlyCounts(buildHourlyCounts(currentEvents, now));
            insights.setRecentEvents(currentEvents.stream().sorted(Comparator.comparingLong((LoggedEvent event) -> event.timestamp).reversed()).limit(12).map(this::toDto).toList());
            return insights;
        }
    }
    ...
    private List<GatewayHourlyLogCountResponse> buildHourlyCounts(List<LoggedEvent> currentEvents, long now) {
        List<GatewayHourlyLogCountResponse> buckets = new ArrayList<>();
        ...
        for (int offset = 23; offset >= 0; offset -= 1) {
            ...
            GatewayHourlyLogCountResponse bucket = new GatewayHourlyLogCountResponse();
            bucket.setHourLabel(HOUR_LABEL_FORMATTER.format(Instant.ofEpochMilli(bucketStart).atZone(ZoneId.systemDefault())));
            bucket.setWarnCount(currentEvents.stream().filter(event -> event.timestamp >= bucketStart && event.timestamp < bucketEnd && "WARN".equals(event.level)).count());
            bucket.setErrorCount(currentEvents.stream().filter(event -> event.timestamp >= bucketStart && event.timestamp < bucketEnd && "ERROR".equals(event.level)).count());
            buckets.add(bucket);
        }
        return buckets;
    }

    private GatewayLogEventResponse toDto(LoggedEvent event) {
        GatewayLogEventResponse dto = new GatewayLogEventResponse();
    ...
    private String categorize(ILoggingEvent eventObject) {
        ...
        if (containsAny(base, "configserver", "config server", "propertysource", "release-on-vm", "deploy", "artifact", "nginx")) {
            return "Configuration";
        }
        if (containsAny(base, "sql", "jdbc", "hibernate", "datasource", "hikari")) {
            return "Database";
        }
        if (containsAny(base, "jwt", "oauth", "token", "auth", "security")) {
            return "Auth";
        }
